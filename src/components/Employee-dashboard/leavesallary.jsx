...
import { FaBars, FaFileInvoiceDollar, FaCalendarAlt, FaArrowLeft } from "react-icons/fa";
...
const SalaryLeaveManagement = () => {
  ...
  useEffect(() => {
    ...
    // Allow only months up to the current month
  ...
  useEffect(() => {
    const fetchLeaveRequest = async () => {
      try {
        const response = await fetch("http://localhost/login-backend/check_leave_request.php");
  ...
  const updateAvailableMonths = (year) => {
    if (year === "2025") {
      // Only allow months up to the current month in 2025
      const currentMonth = new Date().getMonth(); // 0-based index
      setAvailableMonths(monthsList.slice(0, currentMonth + 1));
    } else {
      // For 2024 and 2023, show all months
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost/login-backend/leave_management.php", {
        method: "POST",
        body: formData,
      });
  ...
  const handleMonthChange = (e) => {
    ...
    setSelectedMonth(month); // This will set the selected month as "Month-YYYY"
  };
  ...
  const handleSearchClick = async () => {
    ...
    // Make sure the month is in "January-2025" format
    const formattedMonth = selectedMonth; // This is already in "Month-YYYY" format
  
    try {
      const response = await fetch(
        `http://localhost/login-backend/salary_slip.php?month=${formattedMonth}`
      );
      const data = await response.json();
  ...
  // In your React component, use this `handleSearchClick` function for the Search button
...
{/* Year Dropdown (Fixed to 2025) */}
{/* Year Dropdown (Now includes 2023, 2024, 2025) */}
<div className="flex flex-wrap items-center gap-4 mb-4">
  <label className="text-lg font-semibold text-gray-700">Year:</label>
  <select
    className="bg-white border border-gray-300 rounded-lg p-2"
    value={selectedYear}
    onChange={handleYearChange} // Allow changing the year
  >
...
<div className="flex flex-wrap items-center gap-4 mb-4">
  ...
  >
    ...
    {salaryDetails && (
      <div className="mt-6">
        <h3 className="text-xl font-bold text-gray-700 mb-3">Salary Details for {selectedMonth}</h3>
        <table className="w-full border-collapse border border-gray-300">
          ...
          <tbody>
            <tr>
              ...
              <td className="border p-2">{salaryDetails.ifsc_code}</td>
            </tr>
          </tbody>
        </table>
      </div>
            ...
            {activeSection === "leave" && (
              <>
                    ...
                    <table className="min-w-full table-auto">
                      ...
                      <tbody>
                        {leaveRequest.map((request) => (
                          <tr key={request.id}>
                            <td className="px-4 py-2 border">{request.subject}</td>
                            <td className="px-4 py-2 border">{request.content}</td>
                            <td className="px-4 py-2 border">{request.start_date}</td>
                            <td className="px-4 py-2 border">{request.end_date}</td>
                ...
                <div className="mt-8 bg-gray-50 p-6 rounded-lg shadow">
                  ...
                  <p className="text-gray-600 mb-6">Fill out the form below to submit a leave request.</p>
