...
const AssignedProjects = () => {
    ...
    try {
      ...
      if (data.success && data.track_records.status !== "Not Assigned Yet") {
        setReportFormData({
          ...
          progress_percentage:
            data.track_records.progress_percentage.toString(),
          image: null,
          image2: null,
          image3: null,
          ...
          estimated_completion_date:
            data.track_records.estimated_completion_date || "",
        });
      } else {
        setReportFormData({
          challenges: "",
  ...
  const handleReportSubmit = async (e) => {
    ...
    formDataToSend.append(
      "progress_percentage",
      reportFormData.progress_percentage
    );
    if (reportFormData.image)
      formDataToSend.append("image", reportFormData.image);
    if (reportFormData.image2)
      formDataToSend.append("image2", reportFormData.image2);
    if (reportFormData.image3)
      formDataToSend.append("image3", reportFormData.image3);
    formDataToSend.append(
      "summary_work_completed",
      reportFormData.summary_work_completed
    );
    formDataToSend.append("next_steps", reportFormData.next_steps);
    formDataToSend.append(
      "estimated_completion_date",
      reportFormData.estimated_completion_date
    );

    try {
      const response = await fetch(
        "http://localhost/login-backend/submit_project_report.php",
        {
          method: "POST",
          body: formDataToSend,
        }
    ...
    <div className="flex h-screen bg-gradient-to-r from-gray-100 to-gray-200">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 p-4 md:p-6 overflow-y-auto">
          <h1 className="text-2xl md:text-4xl font-bold text-orange-600 mb-4 md:mb-8 text-left tracking-tight">
            Assigned Projects
          </h1>
          {!selectedProject ? (
            error ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {projects.map((project) => (
                  <div
                    key={project.id}
                    className="relative bg-white border border-gray-200 rounded-xl p-4 md:p-5 cursor-pointer shadow-md hover:shadow-xl hover:border-orange-500 transition-all duration-300 transform hover:-translate-y-1"
                    onClick={() => handleProjectClick(project.id)}
                  >
                    <h2 className="text-lg md:text-xl font-semibold text-gray-800 hover:text-orange-600">
            ...
            <div className="w-full p-4 md:p-6 bg-gray-50 rounded-xl shadow-lg">
              <button
                onClick={() => setSelectedProject(null)}
                className="text-gray-700 hover:text-orange-600 mb-4 md:mb-6 flex items-center text-lg font-medium"
              >
                ← Back to Projects
              </button>
              <div>
                ...
                <div className="space-y-3 text-gray-600">
                  ...
                  <p>
                    ...
                    {selectedProject.associated_employees.length > 0
                      ? selectedProject.associated_employees.join(", ")
                      : "None"}
                  </p>
                </div>
                ...
                {showReportForm && (
                  <form
                    ...
                    className="mt-6 bg-white p-4 md:p-6 rounded-lg shadow-md"
                  >
